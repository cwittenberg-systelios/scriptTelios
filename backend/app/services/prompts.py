...
WORKFLOW_INSTRUCTIONS_DEFAULT: dict[str, str] = {
    ...
    "entlassbericht": (
        ...
        "mit den konkreten Werten referenzieren (nur Aufnahmewerte ohne "
        "Entlasswerte: Testwerte weglassen).\n\n"
    ...
    "sns_verlauf": (
        ...
        "1. Zusammenfassung: 3 Absätze - Verlaufsgestalt mit Datum des "
        "Ordnungsübergangs und des DK-Maximums; was inhaltlich am Übergang "
        "geschah (mit Zitat); Bilanz Anfang → Ende der Gruppen (Symptome, "
        "Selbstwirksamkeit, Zielerleben) und die tragenden ISM-Faktoren.\n"
        "2. Fragebögen und Faktorstruktur: HSF-Basisbogen in 2 Sätzen; das "
        "Systemmodell des individuellen Bogens in den WORTEN DER KLIENTIN/DES "
        "KLIENTEN (Itemtexte sinngemäß: Kernanliegen, Ressource, Hindernis, "
        "Gewinn, Preis); unbesetzte Faktoren; automatische Polungskorrektur.\n"
        "3. Verlauf der Kernfaktoren: Kopplung der Faktoren; REIHENFOLGE des "
        "Wandels (welche Gruppe/Items früh, welche erst am Übergang springen - "
        "aus 'Größte Sprünge'); Symptomgipfel mit ihren Auslösern "
        "(Auslöser-Kategorien, je Muster ein Zitat).\n"
        "4. Individueller Fragebogen: tragender Faktor (größter Sprung), Anker "
        "in der Krise, langsamster Faktor als Fokus der Weiterbehandlung; "
        "Entwicklung in den Kommentaren mit Zitaten; Plateau-Tage, falls "
        "geflaggt.\n"
        "5. Dynamische Komplexität: Anstieg, Maximum und Resonanz vor dem "
        "Übergang; ZWEITER DK-Gipfel mit seinen Ereignissen, falls vorhanden; "
        "DK am Ende und letzter Tag mit kritischem Item; Deckeneffekt.\n"
        "6. Rekurrenzmuster: Heterogenität Anfangsblock vs. Endblock, "
        "Anfang↔Ende als ATTRAKTORWECHSEL, Lage der Übergangsphase zu "
        "Anfang/Ende, Einbruchstage im alten oder neuen Zustandsraum.\n"
        "7. Phasen: je Phase ein Satz 'P1 – Name: …'.\n"
        "8. Anfang und Ende: Kernaussage in 2-3 Sätzen.\n"
        "9. Einordnung: Unterabschnitte als eigene Absätze, jeweils mit fett "
        "gesetztem Einstieg: **Therapeutische Beziehung und Rahmen.** "
        "**Medikation als Kontextfaktor.** (nur wenn Medikation im Faktenblock) "
        "**Deckeneffekte am Ende.** (nur bei Flag) **Frühwarnzeichen für die "
        "Zeit nach der Entlassung.** (Kontrollparameter = Auslösekategorien, "
        "nummeriert im Satz; Frühwarnzeichen = Einbruch länger als 2 Tage "
        "oder erneuter DK-Anstieg; SNS-Monitoring ambulant fortsetzen) "
        "**Individueller Fragebogen nach der Entlassung.** (für jeden "
        "unbesetzten Faktor EIN konkreter Itemvorschlag in Anführungszeichen, "
        "ein zweites Item für den langsamsten Faktor, ereignisbezogene Items "
        "umformulieren) **Offene Themen.** (inkl. neuer Somatik)\n\n"
        "EINBRÜCHE nach Auslösekategorien gruppieren; ein Muster nur benennen, "
        "wenn es sich in Zitaten UND in den Items zeigt."
    ),
}
...
WORKFLOW_INSTRUCTIONS_EB_THEMATISCH: str = (
    ...
    "liegen nur Aufnahmewerte ohne Entlasswerte vor, Testwerte ganz weglassen.\n\n"
...
BASE_PROMPTS: dict[str, str] = {
    ...
    "anamnese": (
        ...
        # v19.26 (A3c): zirkulaere Begruendung ausschliessen. Log 08.09.2026
        # (Job cae59639): "Gruebeln im Rahmen einer PTBS" - die Diagnose
        # erklaert das Symptom, das sie begruenden soll.
        "VERBOTEN ist insbesondere die Diagnose als Erklärung der Beschwerden "
        "(zirkulär): NICHT 'Flashbacks und Grübeln im Rahmen einer PTBS', "
        "NICHT 'vor dem Hintergrund einer depressiven Episode', NICHT 'aufgrund "
        "ihrer Angststörung', NICHT 'bedingt durch', 'infolge', 'als Ausdruck "
        "der', 'typisch für' + Diagnose. STATTDESSEN die Beschwerde selbst "
        "beschreiben: was, seit wann, ausgelöst wodurch, wie häufig, wie stark "
        "beeinträchtigend - z.B. 'Hinzu kommen sich aufdrängende Erinnerungen "
        "an den Unfall mit Schreckhaftigkeit und ein anhaltendes Grübeln über "
        "die Trennung, das den Schlaf weiter verkürzt.' (nur quellengedeckt).\n"
        "AUSNAHME - frühere Diagnosen aus den Quellen (Vorbefund, Selbstauskunft, "
        "Vorbehandler): Sie dürfen genannt werden, aber NUR attribuiert und in "
        "der Vergangenheitsform im Zusammenhang der Vorbehandlungen ('Laut "
        "Vorbefund wurde 2019 eine PTBS diagnostiziert', 'die Vortherapeutin "
        "äußerte den Verdacht auf ADHS'), nie als Ursache aktueller Beschwerden.\n\n"
        "NICHT SCHREIBEN:\n"
    ...
    "entlassbericht": (
        ...
        "Quellen keine Testwerte, entfällt der Satz ersatzlos. "
        # v19.28.3: Feedback 24.09. (Herr N.) - nur Aufnahmewerte ohne
        # Entlasswerte -> kein Satz zu Testwerten; die Schwere der Prae-Werte
        # allein zu referieren hat im Verlaufsteil keinen Sinn.
        "Liegen NUR Aufnahmewerte (prä) ohne Entlasswerte (post) vor, "
        "entfällt der Testwerte-Satz ebenfalls komplett – KEINE Schweregrad-"
        "Einordnung allein aus Aufnahmewerten. Die Zahlenwerte "
    ...
    "sns_verlauf": (
        ...
        "ABSCHNITTSREGELN: Tabellen und Abbildungen werden deterministisch "
        "ergänzt - nicht nachbauen. Abschnitt 4 entfällt inhaltlich, wenn kein "
        "individueller Bogen vorliegt (dann ein Satz). Abschnitt 7: je Phase "
        "EIN Satz in der Form 'P1 – Name: Satz.' (Grenzen vorgegeben, du "
        "vergibst nur Namen). In Abschnitt 9 beginnt jeder Absatz mit einem "
        "fett gesetzten Einstieg (**…**); das ist das einzige Markdown im Text.\n\n"
        ...
        "Prozentangaben, die nicht im Faktenblock stehen. 'signifikant' nur "
        "zusammen mit dem p-Wert aus dem Faktenblock; die Reihen sind kurz, "
        "Trends deskriptiv formulieren. ZITATREGEL: Zitate nur aus den "
        "Stage-A-Ereignissen oder der Medikationszeile, wörtlich, in "
        "Anführungszeichen, mit Datum.\n\n"
        "FLAGS im Faktenblock zwingend aufgreifen. Medikation: Datum der "
        "Umstellung (bei 'wirksam ab' dieses Datum) und zeitliche Nähe zum "
        "Übergang benennen, eine pharmakologische Kurzfristwirkung "
        "relativieren, weitere Medikationsereignisse (z. B. Ablehnung einer "
        "Erhöhung) einordnen. Neue Somatik: Hinweis auf Abklärung bzw. "
        "Entlassbrief. Suizidalität/Selbstverletzung NUR erwähnen, wenn der "
        "Faktenblock sie aus dem Tagebuch meldet (dann in Abschnitt 9 als "
        "Hinweis an die Behandler:innen); sonst gar nicht, auch keine "
        "Negativaussage.\n\n"
        "STIL: systemische Prosa in ganzen Absätzen, KEINE Aufzählungen, keine "
        "Tabellen. Klientin/Klient wie im Faktenblock vorgegeben (\"die "
        "Klientin (Kürzel)\" bei der ersten Nennung, danach \"die Klientin\"; "
        "Klientin/Klient immer groß). Keine neuen Diagnosen, keine Bewertung "
        "der Person, Entwicklungsrichtung statt Defizit. Das Wort "
        "'hypnosystemisch' kommt nicht vor.\n\n"
        "DATENSCHUTZ: Keine Namen - weder von Angehörigen noch von "
        "Mitklient:innen noch von Therapeut:innen; Rollen verwenden (Partner, "
        "Tochter, Mitklientin, die Therapeutin).\n\n"
        "BEISPIEL FÜR TON UND DICHTE (erfundener Fall, keine Zahlen daraus "
        "übernehmen):\n"
        "\"Auffällig ist die Reihenfolge des Wandels: Die Selbstregulation "
        "wächst schon in der ersten Woche langsam, während Selbstwert und "
        "Selbstwirksamkeit bis zum Übergang am Boden bleiben und erst dann "
        "sprunghaft ansteigen. Das passt zur Selbstbeschreibung des Klienten, "
        "der zuerst lernt, „einen Schritt zurückzutreten“, und die Überzeugung, "
        "es selbst zu können, erst mit dem konkreten Erfolgserlebnis gewinnt.\"\n"
        "\"**Frühwarnzeichen für die Zeit nach der Entlassung.** Aus den Daten "
        "lassen sich zwei Kontrollparameter ablesen: erstens Konflikte am "
        "Arbeitsplatz, zweitens Wochenenden allein. In der Krise hielten "
        "Einbrüche mehrere Tage an, später erholte er sich nach einem Tag; ein "
        "Einbruch über zwei Tage ist deshalb das zentrale Warnzeichen.\""
    ),
}
